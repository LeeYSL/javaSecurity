...
public class DigesMain1 {
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		plain = str.getBytes(); //배열에 넣어서 반환해준다는 뜻임>??>?>?>
		for(String al : algo) {
			MessageDigest md = MessageDigest.getInstance(al); //해쉬 알고리즘 객체
			hash = md.digest(plain); //해쉬값
			System.out.println(al + "해쉬값 크기:" +(hash.length*8) + "bits"); //512비트 => 64바이트
			System.out.println("해쉬값:");
			for(byte b : hash) System.out.printf("%02X",b); //16진수 2자리 코드 값으로 출력
			System.out.println();
		}
